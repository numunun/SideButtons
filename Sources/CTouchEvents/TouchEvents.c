 ...
 */
 ...
 /*
 *  Modified 2026-09-03 by numunun for the SideButtons project.
 *  Change: tl_uptime() now uses clock_gettime_nsec_np(CLOCK_UPTIME_RAW)
 *  instead of Carbon's UpTime() + AbsoluteToNanoseconds(). The original
 *  implementation is retained in a comment at the call site.
 *  No other changes were made to this file.
 *
 *  This file remains licensed under the GNU General Public License,
 *  version 2 or later, as received from Calf Trail Software, LLC.
 */
...
// 원본은 Carbon의 UpTime() + AbsoluteToNanoseconds()를 썼는데, CoreServices에
// 의존하고 오래전에 deprecated 됐다. clock_gettime_nsec_np(CLOCK_UPTIME_RAW)가
// 문서화된 현대적 등가물이다: 부팅 이후 나노초, 슬립 중에는 증가하지 않음.
// 주의 - mach_absolute_time()으로 바꾸면 안 된다. Apple Silicon의 timebase는
// 1:1이 아니라 125/3이라서 값이 틀어진다.
//
